...
@interface FishTabPagerController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    struct {
...
#define kUnderLineViewHeight 0

@implementation FishTabPagerController
...
- (void)configireTabPropertys
{
    ...
    _normalTextFont = [UIFont systemFontOfSize:25];//改动过的
    _selectedTextFont = [UIFont systemFontOfSize:30];//改动过的
...
- (void)setDelegate:(id<FishTabPagerControllerDelegate>)delegate
{
...
- (void)setDataSource:(id<FishPagerControllerDataSource>)dataSource
{
    [super setDataSource:dataSource];
    
    _tabDataSourceFlags.titleForIndex = [self.dataSource respondsToSelector:@selector(pagerController:titleForIndex:)];
    NSAssert(_tabDataSourceFlags.titleForIndex, @"FishPagerControllerDataSource pagerController:titleForIndex: not impletement!");
}
